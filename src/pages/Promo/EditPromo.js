...
class EditPromo extends Component {
  ...
  saveHandler = async () => {
    ...
    if (this.state.sendImg !== null) {
      body.append("image", this.state.sendImg, this.state.sendImg.name);
    }
    try {
        ...
        <main className="container" style={{ marginTop: 38, marginBottom: 44 }}>
          ...
          <section className="flex-between">
              ...
              <div style={{ position: "relative", top: "30px" }}>
                ...
                <div className="date">
                  <input
                    type="date"
                    className="input-box"
                    value={`${showStartPromo}`}
                    onChange={(e) =>
                      this.setState({ starPromo: e.target.value })
                    }
                  />
                  <input
                    type="date"
                    ...
                    style={{ marginTop: 20 }}
                    onChange={(e) =>
                      this.setState({ endPromo: e.target.value })
                    }
                  />
                </div>
                <p className="label" style={{ marginTop: 47 }}>
                  Input promo code :
                </p>
            ...
            <div className="form-edit">
              ...
              />
              <p className="label" style={{ marginTop: 41 }}>
                Normal Price :
              </p>
              <input
                type="number"
                ...
                onChange={(e) => this.setState({ normalPrice: e.target.value })}
              />
              <p className="label" style={{ marginTop: 41 }}>
                Description :
              </p>
              ...
              <p className="click-text">
                Click methods you want Click product size you want to use for
                this promo
              </p>
              ...
              <p className="label" style={{ marginTop: 54 }}>
              ...
              <p className="label" style={{ marginTop: 73 }}>
                Enter the discount :
              </p>
              <select
                name="discount"
                value={this.state.selectDiscount}
                className="input-box select"
                style={{ marginTop: 28 }}
                onChange={(e) =>
                  this.setState({ selectDiscount: e.target.value })
                }>
                {arr.map((value) => (
                  <React.Fragment key={value}>
                    <option value={value}>{`${value}% -> ${(value / 100) *
                      this.state.normalPrice}`}</option>
                  </React.Fragment>
                ))}
              </select>
